...
import { ROOT_PATH, childPath, nodeLabel, parentPath } from "~/utils/nodePath";
...
export function nearestCommonAncestor(paths: string[]): string {
  ...
  // A node's own path is its parent + its own label; the common ANCESTOR must
  // stop before any node's final label, so trim to at most (len-1) of shortest.
  const minLen = Math.min(...split.map((s) => s.length));
  const depth = Math.min(i, minLen - 1);
  return first.slice(0, Math.max(1, depth)).join(".") || ROOT_PATH;
}

// Re-root every subtree in `flat` whose path is `oldRoot` (or under it) onto
// `newRoot`, preserving descendant structure.
function reroot(
  flat: NodeModel[],
  oldRoot: string,
  newRoot: string,
  ...
  return flat.map((node) => {
    ...
    if (node.path.startsWith(`${oldRoot}.`))
